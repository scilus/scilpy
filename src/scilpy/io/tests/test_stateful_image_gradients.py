...
def test_validation_errors():
    with create_dummy_nifti_with_gradients(n_volumes=5) as (img_p, bval_p, bvec_p, bvals, bvecs):
        simg = StatefulImage.load(img_p)
        
        # Wrong number of volumes
        with pytest.raises(ValueError, match="Number of gradients.*does not match number of volumes"):
...
def test_gradient_consistency_across_orientations():
    ...
    """
    n_volumes = 4
    with create_dummy_nifti_with_gradients(n_volumes=n_volumes) as (img_p, bval_p, bvec_p, bvals, bvecs):
        ...
        for target_ornt in ["LAS", "LPS", "LPI"]:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                simg_target = StatefulImage.convert_to_simg(simg_ras, simg_ras.bvals, simg_ras.bvecs)
                ...
                simg_verify = StatefulImage.load(target_img_p, to_orientation="RAS")
                ...
                assert np.allclose(simg_verify.bvecs, original_bvecs, atol=1e-5)
