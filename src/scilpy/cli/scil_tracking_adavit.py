...
def _build_arg_parser():
    ...
    track_g.add_argument('--dilate_tracking_mask', type=int, default=0,
                         help="Dilate the tracking mask a given number of times. [%(default)s]")
    ...
    backtrack_g.add_argument('--mask_exclude',
                            help='Exclusion mask. Streamlines terminating inside \n'
                                 'this mask will be discarded.')
    backtrack_g.add_argument('--backtrack_distance', type=float, default=2.0,
                             help='Distance from endpoint to backtrack when\n'
                                  'tracking gets stuck, in mm. [%(default)s]')
    backtrack_g.add_argument('--mask_interp', default='nearest',
                             choices=['nearest', 'trilinear'],
...
def main():
    ...
    if not mask_img.ndim == 4:
        parser.error('The tracking mask must be a 4D volume for tracer-informed tractography.')

    # 4D volume for tracer-informed tractography
    mask = np.asarray(mask_img.dataobj)  # this ensures we don't cast the masks to float
    if args.dilate_tracking_mask > 0:
        logging.info(f"Dilating tracking mask with {args.dilate_tracking_mask} iterations.")
        for i in tqdm_if_verbose(np.arange(mask.shape[3]), args.verbose):
            mask[..., i] = binary_dilation(mask[..., i], iterations=args.dilate_tracking_mask)
    ...
    mask_exclude = None
    if args.mask_exclude:
        logging.info("Loading exclusion mask.")
        exclude_img = nib.load(args.mask_exclude)
        exclude_data = exclude_img.get_fdata(caching='unchanged', dtype=float)
