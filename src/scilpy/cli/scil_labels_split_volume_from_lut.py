...
from scilpy.image.labels import get_data_as_labels, split_labels, SCILPY_LUT_DIR
...
def _build_arg_parser():
    luts = [os.path.splitext(f)[0] for f in os.listdir(SCILPY_LUT_DIR)]
...
def main():
    ...
    if args.scilpy_lut:
        with open(os.path.join(SCILPY_LUT_DIR, args.scilpy_lut + '.json')) as f:
