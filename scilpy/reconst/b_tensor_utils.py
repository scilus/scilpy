...
def generate_btensor_input_new(input_files, bvals_files, bvecs_files,
    ...
    for inputf, bvalsf, bvecsf, b_delta in zip(input_files, bvals_files,
        ...
        if inputf:
            ...
            for ubval in ubvals: # Loop over all unique bvals
                ...
                if ubval > tol: # All bvals except the b0s
                ...
                elif not ubvals_full.size: # The case that the first bval is b0
                    ubvals_full = np.array([ubval])
                    ub_deltas_full = np.array([1])
                    nb_bvecs_full = np.array([nb_bvecs])
                elif ubvals_full.size:
                    if same_bvals.size: # The case that there is already a b0
                        nb_bvecs_full[same_bvals] += nb_bvecs
                    else: # The case that there is no b0
                        ubvals_full = np.concatenate((ubvals_full, [ubval]))
                        ub_deltas_full = np.concatenate([ub_deltas_full, [1]])
                        nb_bvecs_full = np.concatenate([nb_bvecs_full, [nb_bvecs]])
                # Building all the rest of the data, regardless of b0 or not
    ...
    nb_bvecs_full = np.take_along_axis(np.asarray(nb_bvecs_full),
                                        sorted_indices, axis=0)
    ...
    return(gtab_full, data_full, ubvals_full, ub_deltas_full, nb_bvecs_full)
