...
def test_tractogram_pairwise_comparison():
    ...
    assert np.mean(results[0][~np.isnan(results[0])]) == 0.7526672821759841
    assert np.mean(results[1][~np.isnan(results[1])]) == 0.2573811906676235
    assert np.mean(results[2][~np.isnan(results[2])]) == 0.6381088710917843
    assert np.mean(results[3][~np.isnan(results[3])]) == 0.7035940807380183
