...
def test_merge_dict_recursive():
    assert(expected_merged_dict_12_recursive == merge_dict(dict_1, dict_2, recursive=True))

def test_merge_dict_all_true():
    assert(expected_merged_dict_12_all_true == merge_dict(dict_1, dict_2, no_list=True, recursive=True))
