...
def main():
    ...
    avafodf_data = avafodf_img.get_fdata(dtype=float)

    fodf_img = nib.load(args.in_fodf)
    fodf_data = fodf_img.get_fdata(dtype=float)

    nonzero_fodf = get_data_as_mask(nib.load(args.in_mask), dtype=bool)
    ...
    nib.save(nib.Nifti1Image(odd_pwr_map.astype(float),
