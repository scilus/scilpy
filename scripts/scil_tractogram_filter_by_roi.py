...
def prepare_filtering_list(parser, args):
    ...
    if (len(roi_opt_list[-1]) < 4 or len(roi_opt_list[-1]) > 5) and roi_opt_list[-1][0] != 'atlas_roi':
        logging.error("Please specify 3 or 4 values "
                      "for {} filtering.".format(roi_opt_list[-1][0]))
    elif (len(roi_opt_list[-1]) < 5 or len(roi_opt_list[-1]) > 6) and roi_opt_list[-1][0] == 'atlas_roi':
