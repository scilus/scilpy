...
def compute_voxel_measures(args):
    ...
    if not os.path.isfile(bundle_filename):
        print('{0} does not exist'.format(bundle_filename))
        return None
    ...
    _, bundle_dimensions, _, _ = bundle_sft.space_attribute

    if not bundle_streamlines:
        print('{0} is empty'.format(bundle_filename))
        return None
...
def main():
    ...
    if args.streamlines_measures:
        ...
        _, gs_dimensions, _, _ = gs_sft.space_attribute
