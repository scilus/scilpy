...
def merge_metrics(acc, corr, diff, beta=1.0):
    ...
    masked_acc, masked_corr, masked_diff = [ma.masked_array(x, mask) for x in [acc, corr, diff]]
...
def main():
    ...
    sft_1.to_corner()
    todi_obj = TrackOrientationDensityImaging(tuple(dimensions), 'repulsion724')
    ...
    with ProcessPoolExecutor(max_workers=nbr_cpu) as executor:
        futures = {executor.submit(compute_difference_for_voxel, chunk): chunk for chunk in index_chunks}
