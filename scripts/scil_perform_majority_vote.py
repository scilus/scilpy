...
def _build_args_parser():
    ...
    p.add_argument('--ratio_streamlines', type=int, default=2,
                   help='Minimum vote to be considered for streamlines '
                   '[%(default)s]')
    p.add_argument('--ratio_voxels', type=int, default=2,
...
def main():
    ...
    for name in args.bundles:
        fusion_streamlines.extend(load_tractogram_with_reference(parser,
                                                                 args,
                                                                 name))
    if args.reference:
    ...
    for i, name in enumerate(args.bundles):
        ...
        bundle = list(sft.streamlines)
        sft.to_vox()
        bundle_vox_space = list(sft.streamlines)
    ...
    if args.same_tractogram:
        ...
        for i in range(len(fusion_streamlines)):
            if np.sum(streamlines_vote[i]) >= args.ratio_streamlines:
    ...
    volume[volume < args.ratio_voxels] = 0
    volume[volume >= args.ratio_voxels] = 1
