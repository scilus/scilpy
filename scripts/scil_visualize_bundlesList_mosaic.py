...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description='Visualize bundles from a list of bundles. The script will output '
                    'one screenshot per direction (6 total) per each bundles of the list.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('anat_reference',   help='Image used as background (e.g. T1, FA, b0)')
    parser.add_argument('inputs',       action='store',  nargs='+',  help='List of streamline files supported by nibabel')
    parser.add_argument('output_name',  help='Name of the output image mosaic (e.g. jpg, png)')
    parser.add_argument('--zoom',       type=float, default=1.0, help='Rendering zoom. A value greater than 1 is a zoom-in, a value less than 1 is a zoom-out.')
    parser.add_argument('--ttf',        type=str, default=None,  help='Path of the true type font to use for the legends')
    parser.add_argument('--ttf_size',   type=int, default=35,    help='Font size to use for the legends')
...
def main():
    ...
    if args.ttf is not None:
        font    = ImageFont.truetype('DejaVuSerif.ttf', args.ttf_size)
    else:
    ...
    for num, name in enumerate(output_names):
        ...
        draw.text((i + text_pos_x, j + text_pos_y + 50), name[1+name.find('_'):],  font=font)
    ...
    draw.text((i + text_pos_x, j + text_pos_y + 50), ('Streamlines'),  font=font)    
    ...
    for numBundle, bundle_file in enumerate( listOfBundles ):
        ...
        if not os.path.isfile(bundle_file):
            ...
            draw.text((i + text_pos_x, j + text_pos_y),      (bundle_file_name),  font=font)
            draw.text((i + text_pos_x, j + text_pos_y + 50), ('%d'%stats[bundle_name]['number_streamlines']),  font=font)
