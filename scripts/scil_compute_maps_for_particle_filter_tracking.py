...
def main():
    ...
    logging.info('"{0}" loaded as WM PVE map.'.format(args.in_wm))
    gm_pve = nib.load(args.in_gm)
    logging.info('"{0}" loaded as GM PVE map.'.format(args.in_gm))
    csf_pve = nib.load(args.in_csf)
    logging.info('"{0}" loaded as CSF PVE map.'.format(args.in_csf))
    ...
    nib.Nifti1Image(include_map.astype(np.float32),
                    gm_pve.affine).to_filename(args.include)
    nib.Nifti1Image(exclude_map.astype(np.float32),
                    gm_pve.affine).to_filename(args.exclude)
    nib.Nifti1Image(interface.astype(np.float32),
